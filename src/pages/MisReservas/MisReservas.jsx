  ...
  return (
    <div className="p-5 flex flex-col sm:flex-row sm:items-start gap-4">
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        <button
          onClick={() => navigate(`/libros/${reserva.libro_id}`)}
...
export default function MisReservas() {
  ...
  const cargar = useCallback(() => getReservasByMiembro(user.id), [user.id])
  const [reservas,   setReservas]   = useState(cargar)
  const [cancelando, setCancelando] = useState(null)

  const handleCancelar = async (reservaId) => {
    setCancelando(reservaId)
    await new Promise(r => setTimeout(r, 350))
    const result = cancelarReserva(reservaId)
    setCancelando(null)
    showToast(result.mensaje, result.ok ? 'success' : 'error')
    if (result.ok) setReservas(cargar())
  }
    ...
    <div className="page-container">
      ...
      <div className="mb-6">
        ...
        <p className="page-subtitle">
          Libros que has reservado y tu posición en la cola de espera
        </p>
      </div>
