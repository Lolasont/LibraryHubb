    ...
    <div className="card p-6 flex flex-col gap-5">
      ...
      <div className="pt-1">
        {disponible ? (
          ...
          <button
            disabled
            className="w-full py-2.5 px-5 rounded-lg bg-slate-100 text-slate-400 font-semibold text-sm cursor-not-allowed"
          >
...
export default function LibroDetalle() {
  ...
  const cargarDatos = useCallback(() => {
    ...
    setLibro(l)
    const res = getReservasByLibro(l.id)
    setReservas(res)
    if (user?.rol === 'miembro') {
      const misRes = getReservasByMiembro(user.id)
  ...
  const handlePrestamo = async () => {
    ...
    const result = solicitarPrestamo(user.id, libro.id)
    setLoading(false)
  ...
  const handleReserva = async () => {
    setLoading(true)
    await new Promise(r => setTimeout(r, 400))
    const result = hacerReserva(user.id, libro.id)
    setLoading(false)
    showToast(result.mensaje, result.ok ? 'success' : 'error')
    if (result.ok) cargarDatos()
  }
    ...
    return (
      <div className="page-container">
        <div className="flex flex-col items-center justify-center py-24">
          ...
          <button onClick={() => navigate('/libros')} className="btn-primary mt-5 text-sm">
            Volver al catálogo
          </button>
        </div>
      </div>
  ...
  return (
    <div className="page-container">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="card p-6">
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                ...
                <h1 className="text-xl font-display font-bold text-blue-900 leading-tight">
                  {libro.titulo}
                </h1>
          ...
          <div className="card p-6">
            ...
            <div className="mt-2">
              ...
              <DetalleRow
                ...
                value={`${libro.copias_disponibles} disponibles / ${libro.cantidad_copias} totales`}
              />
            </div>
          </div>
        ...
        <div>
          {esBibliotecario ? (
            // Vista de bibliotecario: solo info, sin botones de acción de miembro
            <div className="card p-6">
